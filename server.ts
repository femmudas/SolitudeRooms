...
const next = require("next") as (opts: { dev: boolean; dir: string }) => {
...
ex.get("/api/drip/pack/:snapshotId", async (req, res) => {
  try {
    ...
    const wallet = z.string().min(32).parse(req.query.wallet);

    const snap = store.getSnapshot(snapshotId);
    if (!snap) {
      res.status(404).send("snapshot_not_found");
      return;
    }

    if (snap.creatorWallet !== wallet) {
      res.status(403).send("only_creator_can_export");
      return;
    }
...
io.on("connection", (socket) => {
  socket.on("room:create", async (payload, cb) => {
    try {
      ...
      cb?.({ ok: false, error: error instanceof Error ? error.message : "create_failed" });
    }
  });
  ...
  socket.on("room:join", (payload, cb) => {
    try {
      ...
      cb?.({ ok: false, error: error instanceof Error ? error.message : "join_failed" });
    }
  });
  ...
  socket.on("chat:send", (payload, cb) => {
    try {
      ...
      cb?.({ ok: false, error: error instanceof Error ? error.message : "send_failed" });
    }
  });
  ...
  socket.on("room:bury", (payload, cb) => {
    try {
      ...
      cb?.({ ok: false, error: error instanceof Error ? error.message : "bury_failed" });
    }
  });
});
